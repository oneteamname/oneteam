...
<c:if test="${!empty profile_img && profile_img eq true}">
	<script type="text/javascript">
		alert("프로필 이미지 등록 성공!");
	</script>
</c:if>
<c:if test="${!empty profile_img && profile_img eq false}">
	<script type="text/javascript">
		alert("프로필 이미지 등록 실패!");
	</script>
</c:if>
<c:if test="${!empty fileCheckResult && fileCheckResult eq false}">
	<script type="text/javascript">
		...
		<div class="navContainer">
			<ul>
				...
				<li style="background-color: rgb(231, 26, 15);"><a
					href="member_MyPage.do?mode=myPageMain" style="color: white;">MyPageHome</a></li>
